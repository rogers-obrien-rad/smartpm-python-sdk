...
def test_get_changes_summary(changes, scenarios, projects):
    """Test retrieving changes summary"""
    ...
    result = changes.get_changes_summary(project_id, scenario_id)
    logger.info("Changes Summary Example: %s", json.dumps(result[0], indent=4))
    
    assert isinstance(result, list)
    assert 'metrics' in result[0]
    assert 'dataDate' in result[0]